...
from .encoding import KINDS, Encoded, Question, batches, collate, encode
...
class Engine:
    ...
    def __init__(self, checkpoint: str | Path, *, device: str | torch.device = "auto", max_len: int | None = None,
                 chunk_tokens: int | None = None, token_budget: int = 16384, threads: int | None = None,
                 gpu_memory_gb: float = 0.0, dtype: torch.dtype | None = None) -> None:
        ...
        self.model, self.tokenizer, self.config = load(checkpoint, max_len)
        self.model.to(self.device).eval()
        self.dtype = dtype or (torch.bfloat16 if self.device.type == "cuda" else torch.float32)
        self.max_len = int(self.config.get("max_len") or WINDOW)
        self.chunk_tokens = self.config.get("chunk_tokens", 0) if chunk_tokens is None else chunk_tokens
        self.token_budget = token_budget
        self.temperatures = dict(self.config.get("temperature_by_options", {}))
    ...
    def decide(self, state: Any, questions: Mapping[str, Mapping[str, Any]]) -> dict[str, Answer]:
        ...
        for qid, raw in questions.items():
            ...
            item = encode(self.tokenizer, question, self.max_len, self.chunk_tokens)
            if item is None:
    ...
    def info(self) -> dict[str, Any]:
        return {"arch": self.config.get("arch"), "encoder": self.config.get("encoder"), "max_len": self.max_len,
                "chunk_tokens": self.chunk_tokens, "device": str(self.device),
